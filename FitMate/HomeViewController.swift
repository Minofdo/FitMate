...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, ExerciseDelegate {
    ...
    override func viewDidLoad() {
        ...
        let button = UIBarButtonItem(title: nil, image: UIImage(systemName:"gear.circle"), target: self, action: #selector(showConfigure))
        navigationItem.rightBarButtonItem = button
        
        setUpView()
        
        if let data = UserDefaults.standard.data(forKey: "SAVED_EXERCISES") {
            let exerciseList = try? PropertyListDecoder().decode([Exercise].self, from: data) 
